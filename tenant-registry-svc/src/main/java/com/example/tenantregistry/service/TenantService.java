...
public class TenantService {
     ...
     * @throws IllegalArgumentException if a tenant with the same identifier already exists
     */
    @Transactional
    public TenantDTO createTenant(CreateTenantRequest request) {
        log.info("Creating tenant with identifier: {}", request.getIdentifier());
        
        // Check if tenant already exists
        if (tenantRepository.existsByIdentifier(request.getIdentifier())) {
            throw new IllegalArgumentException("Tenant with identifier " + request.getIdentifier() + " already exists");
        }
        
        // Create Keycloak realm
        String realmId = request.getIdentifier();
        boolean realmCreated = keycloakService.createRealm(realmId, request.getName());
        if (!realmCreated) {
            log.warn("Keycloak realm already exists for tenant: {}", request.getIdentifier());
        }
        
        // Create database schema
        String schemaName = request.getIdentifier().replace("-", "_");
        boolean schemaCreated = liquibaseService.createSchema(schemaName);
        if (!schemaCreated) {
            throw new RuntimeException("Failed to create database schema for tenant: " + request.getIdentifier());
        }
    ...
    public Page<TenantDTO> getTenants(Predicate predicate, Pageable pageable) {
        return tenantRepository.findAll(predicate, pageable)
                .map(tenantMapper::toDto);
    }
