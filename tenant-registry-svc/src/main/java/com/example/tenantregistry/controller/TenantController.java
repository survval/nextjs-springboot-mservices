...
public class TenantController {
    ...
    @Operation(summary = "Create a new tenant", description = "Creates a new tenant with Keycloak realm and database schema")
    ...
    public Page<TenantDTO> getTenants(
            @QuerydslPredicate(root = Tenant.class) Predicate predicate,
            @ParameterObject @PageableDefault(size = 20) Pageable pageable) {
        return predicate != null
                ? tenantService.getTenants(predicate, pageable)
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete a tenant", description = "Deletes a tenant, including its Keycloak realm and database schema")
